...
def projectCylinderTorch(position, direction, radius, fx, fy, cx, cy):
    ...
    alpha1 = (1-direction[:,0]*direction[:,0])*position[:,0] - direction[:,0]*direction[:,1]*position[:,1] \
                - direction[:,0]*direction[:,2]*position[:,2]
    beta1  = -direction[:,0]*direction[:,1]*position[:,0] + (1 - direction[:,1]*direction[:,1])*position[:,1] \
                - direction[:,1]*direction[:,2]*position[:,2]
    gamma1 = -direction[:,0]*direction[:,2]*position[:,0] - direction[:,1]*direction[:,2]*position[:,1] \
    ...
    C = position[:,0] * position[:,0] + position[:,1] * position[:,1] + position[:,2] * position[:,2] \
        - th.square(position[:,0] * direction[:,0] + position[:,1] * direction[:,1] + position[:,2] * direction[:,2]) \
        - radius * radius
...
def transform_points(position, transform, intr):     
        ...
        position_h = th.cat((position, th.ones(position.shape[0], 1, dtype=position.dtype, device=position.device)), dim=-1)  # (B, 4)
        ...
        projected_points_2d = projected_points_h[:, :2] / projected_points_h[:, 2].unsqueeze(-1)
...
def transform_points_b(position, transform, intr):
    ...
    transformed_position_h = th.matmul(transform, position_h.unsqueeze(-1)).squeeze(-1)  # (B, 4)
    # Convert back to non-homogeneous coordinates (drop the last column)
    transformed_position = transformed_position_h[:, :3]  # (B, 3)
    camera_frame_coors = transformed_position
    # Project the transformed 3D points to 2D image coordinates using camera intrinsics
    ...
    projected_points_h = th.matmul(intr, transformed_position.T).T  # Shape (B, 3)
    # Normalize by z-coordinate to get (u, v) in image coordinates
    projected_points_2d = projected_points_h[:, :2] / projected_points_h[:, 2].unsqueeze(-1)  # (B, 2)
...
if __name__ == '__main__': 
    ...
    def parseArgs(): 
        ...
        parser.add_argument('-md', '--mesh_dir', type=str, default='urdfs/dVRK/meshes', 
                help='directory to mesh files')
        parser.add_argument('-rf', '--ref_img_file', type=str,  
    ...
    def parseCtRNetArgs(): 
        ...
        args.height = 480
        args.width = 640
        args.fx, args.fy, args.px, args.py = 882.99611514, 882.99611514, 445.06146749, 190.24049547
        args.scale = 1.0
    ...
    intr = th.tensor([[ctrnet_args.fx, 0, ctrnet_args.px],
            [0, ctrnet_args.fy, ctrnet_args.py],
            [0,  0,  1]], device=joint_angles.device, dtype=joint_angles.dtype)
    ...
    p_img1 = get_img_coords(p_local, R_list[2], t_list[2], pose_matrix.squeeze().to(joint_angles.dtype), intr)
    p_img2 = get_img_coords(p_local, R_list[3], t_list[3], pose_matrix.squeeze().to(joint_angles.dtype), intr)
    p_img = [p_img1, p_img2]
    ...
    position = th.zeros((num_points, 3), dtype=joint_angles.dtype, device=joint_angles.device)  # (B, 3)
    # The direction of the cylinder is aligned along the z-axis
    direction = th.zeros((num_points, 3), dtype=joint_angles.dtype, device=joint_angles.device)
    direction[:, 2] = 1.0  # Aligned along z-axis

    # Use the existing pose_matrix (which is already a 4x4 transformation matrix)
    pose_matrix = pose_matrix.squeeze().to(joint_angles.dtype)  # Ensure pose_matrix is of shape (4, 4)
    ...
    for i in range(proj_position_2d.shape[0]):
        ...
        cv2.circle(rendered_np, (x, y), radius=5, color=(0, 255, 0), thickness=-1)  # Green color for points
        ...
        cv2.arrowedLine(rendered_np, (x, y), (end_x, end_y), color=(0, 255, 0), thickness=3, tipLength=0.15)  # Red arrow for normal
    ...
    e_1, e_2 = projectCylinderTorch(cam_pts_3d_position, cam_pts_3d_norm, radius, ctrnet_args.fx, ctrnet_args.fy, ctrnet_args.px, ctrnet_args.py)
    ...
    valid_indices_line1 = (u_values >= 0) & (u_values < rendered_np.shape[1]) & (v_values_line1 >= 0) & (v_values_line1 < rendered_np.shape[0])
    valid_u_values_line1 = u_values[valid_indices_line1]
    valid_v_values_line1 = v_values_line1[valid_indices_line1]

    valid_indices_line2 = (u_values >= 0) & (u_values < rendered_np.shape[1]) & (v_values_line2 >= 0) & (v_values_line2 < rendered_np.shape[0])
    ...
    plt.plot(valid_u_values_line1, valid_v_values_line1, color='r', linestyle='-', linewidth=4, label='Line 1 (e1)')
    plt.plot(valid_u_values_line2, valid_v_values_line2, color='b', linestyle='-', linewidth=4, label='Line 2 (e2)')
