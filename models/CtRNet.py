...
class CtRNet(torch.nn.Module):
    ...
    def pose_matrix_to_cTr(self, pose_matrix):
        ...
        angle_axis = kornia.geometry.conversions.rotation_matrix_to_angle_axis(rotation_matrix)
    ...
    def setup_robot_renderer(self, mesh_files):
        ...
        robot_renderer = RobotMeshRenderer(
            ...
            robot=None, mesh_files=mesh_files, device=self.device, visibility_flags = self.visibility_flags) #TODO: test
    ...
    def render_single_robot_mask(self, cTr, robot_mesh, robot_renderer):
        ...
        verts_hom = torch.cat([verts, torch.ones((verts.shape[0], 1), device=verts.device)], dim=1)  # (V, 4)
    ...
    def render_robot_mask_batch(self, cTr_batch, robot_mesh, robot_renderer):
        ...
        R_batched = kornia.geometry.conversions.angle_axis_to_rotation_matrix(cTr_batch[:, :3])  # (B,3,3)
        ...
        batched_meshes = robot_mesh.extend(B)  # Now we have B copies of the same geometry.
