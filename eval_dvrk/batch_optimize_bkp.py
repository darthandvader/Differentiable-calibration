...
class BatchOptimize:
    ...
    def __init__(
        ...
        self.cTr_batch = th.nn.Parameter(cTr_batch.clone()).to(self.device)  # shape (B,6)
    ...
    def single_item_loss(
        ...
        # MSE
        mse_val = F.mse_loss(pred_mask_2d * weighting_mask, ref_mask_2d * weighting_mask)

        # distance loss
        dist_val = distance_loss(pred_mask_2d*weighting_mask, self.distance_map*weighting_mask)
        ...
        if ref_kps_2d is not None:
            intr = th.tensor([[882.99611514,   0,         445.06146749],
            ...
            p_img1 = get_img_coords(p_local1, R_list[2], t_list[2], pose_matrix.squeeze().to(self.joint_angles.dtype), intr)
            p_img2 = get_img_coords(p_local2, R_list[3], t_list[3], pose_matrix.squeeze().to(self.joint_angles.dtype), intr)
        ...
        item_loss = 10*ld1*mse_val + ld2*(1e-7)*dist_val + ld3*(1e-5)*app_val + 5*(1e-3)*pts_val + 1*(1e-3)*cylinder_val
        return item_loss, angle_val



    def cylinder_loss_single(self, position, direction, pose_matrix, radius, fx, fy, px, py):  
        ...
        e_1, e_2 = projectCylinderTorch(cam_pts_3d_position, cam_pts_3d_norm, radius, fx, fy, px, py) # [1,2], [1,2]
