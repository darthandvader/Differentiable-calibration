...
import eval_dvrk.plot_utils as plot_utils
from tqdm import tqdm
import pdb
import skfmm
from torch.optim.lr_scheduler import StepLR
from scipy.spatial.distance import directed_hausdorff
from click_pts import get_reference_keypoints
from models.mark_kp import *
from cylinder_projection import projectCylinderTorch, transform_points
from test_line_detection  import detect_lines
...
def angle_between_lines(params_set1, params_set2):
    ...
    cos_theta = dot_products / (norms1 * norms2 + 1e-6)  # Add epsilon for numerical stability
...
def cylinder_loss_params(detected_lines, projected_lines):
    ...
    def line_difference(theta1, rho1, theta2, rho2):
        ...
        delta_theta = th.abs(theta1 - theta2)  # I guess we dont need to handle periodicity here
    ...
    loss_1_0, theta_1_0 = line_difference(theta_det[0:1], rho_det[0:1], theta_proj[0:1], rho_proj[0:1])
    loss_1_1, theta_1_1 = line_difference(theta_det[1:2], rho_det[1:2], theta_proj[1:2], rho_proj[1:2])
    total_loss_1 = loss_1_0 + loss_1_1
    avg_theta_1 = (theta_1_0 + theta_1_1) / 2.0

    # Pairing 2: Det[0]↔Proj[1], Det[1]↔Proj[0]
    loss_2_0, theta_2_0 = line_difference(theta_det[0:1], rho_det[0:1], theta_proj[1:2], rho_proj[1:2])
    loss_2_1, theta_2_1 = line_difference(theta_det[1:2], rho_det[1:2], theta_proj[0:1], rho_proj[0:1])
    ...
    centerline_loss, _ = line_difference(theta_det_mean, rho_det_mean, theta_proj_mean, rho_proj_mean)
    ...
    distance_loss_1 = th.norm(detected_lines[0]-projected_lines[0]) + th.norm(detected_lines[1]-projected_lines[1])
    distance_loss_2 = th.norm(detected_lines[0]-projected_lines[1]) + th.norm(detected_lines[1]-projected_lines[0])
    centerline_loss_distance = th.norm(th.mean(detected_lines, dim=0)-th.mean(projected_lines, dim=0))
...
def keypoint_chamfer_loss(keypoints_a, keypoints_b, pts = True):
    ...
    if keypoints_a.size(0) != 2 or keypoints_b.size(0) != 2:
        raise ValueError("This brute force method only works for exactly two keypoints in each set.")
    
    # Compute distances for both possible permutations:
    # Permutation 1: A0->B0 and A1->B1
    dist_1 = th.norm(keypoints_a[0] - keypoints_b[0]) + th.norm(keypoints_a[1] - keypoints_b[1])
    
    # Permutation 2: A0->B1 and A1->B0
    dist_2 = th.norm(keypoints_a[0] - keypoints_b[1]) + th.norm(keypoints_a[1] - keypoints_b[0])
    ...
    if pts_a.size(0) >= 2 and pts_b.size(0) >= 2:  # Ensure at least two points in each set
...
class Optimize(object): 
    ...
    def readRefImage(self, ref_img_file): 
        ...
        img = cv2.inRange(cv_img, np.ones(3)*128, np.ones(3)*255) / 255.
    ...
    def optimize(
        ...
        angles_over_time = []
        intr = th.tensor([[self.fx,   0,         self.px],
                [  0,         self.fy, self.py],
                [  0,           0,           1       ]], device='cuda', dtype=self.cTr_train[3].dtype)
        ...
        for idx in tqdm(range(iterations), desc="optimizing the pose..."):
            ...
            self.ref_keypoints = th.tensor(self.ref_keypoints, device=self.model.device, dtype=th.float32).squeeze()
            ...
            pose_matrix = self.model.cTr_to_pose_matrix((self.buildcTr(self.cTr_train, self.cTr_nontrain)).unsqueeze(0))
            R_list, t_list = lndFK(self.cTr_train[3])
            R_list = R_list.to(self.model.device)
            t_list = t_list.to(self.model.device)
            p_local1 = th.tensor([0.0, 0.0004, 0.009]).to(self.cTr_train[3].dtype).to(self.model.device)
            p_local2 = th.tensor([0.0, -0.0004, 0.009]).to(self.cTr_train[3].dtype).to(self.model.device)
            p_img1 = get_img_coords(p_local1, R_list[2], t_list[2], pose_matrix.squeeze().to(self.cTr_train[3].dtype), intr)
            p_img2 = get_img_coords(p_local2, R_list[3], t_list[3], pose_matrix.squeeze().to(self.cTr_train[3].dtype), intr)
            proj_pts = th.stack([p_img1, p_img2], dim=0)
             
            # print(f"debugging input shape: {proj_pts.shape}")        
            loss, angle = self._loss(rendered_image, proj_pts, ref_pts, ld1, ld2, ld3, grid_search)
            ...
            # loss.backward()
            if idx < iterations - 1:
                loss.backward(retain_graph=True)  # Retain graph for future backward passes
            else:
            ...
            if (idx+1)% saving_interval == 0 or idx == 0 or idx == iterations-1: 
                if not save_fig_dir is None: 
                    ...
                    overlay = self._overlay_masks(rendered_np, ref_np, ref_pts, proj_pts, det_line_params, proj_line_params)
        ...
        return self.buildcTr(self.cTr_train, self.cTr_nontrain), loss.detach().cpu(), angle.detach().cpu()


    def save_image_cv(self, overlay, save_path):
        overlay_bgr = (overlay * 255).astype(np.uint8)  # Convert to uint8 and BGR format
    ...
    def compute_weighting_mask(self, shape, center_weight=1.0, edge_weight=0.5):
        ...
        weights = edge_weight + (center_weight - edge_weight) * (1 - normalized_distance)
    ...
    def cylinder_loss(self, position, direction, pose_matrix, radius, fx, fy, cx, cy):
        ...
        intr = th.tensor([[fx, 0, cx],
                    [0, fy, cy],
                    [0, 0, 1]], device=self.model.device, dtype=th.float32)
        ...
        e_1, e_2 = projectCylinderTorch(cam_pts_3d_position, cam_pts_3d_norm, radius, fx, fy, cx, cy) # [1,2], [1,2]
        ...
        if self.det_line_params == None:
            ...
            detected_lines = th.from_numpy(np.stack((a, b), axis=-1)).to(self.model.device)  
    ...
    def _loss(self, rendered_img, proj_pts, ref_pts, ld1, ld2, ld3, grid_search): 
        ...
        # weighting_mask = self.compute_weighting_mask(rendered_img.squeeze().shape).to(self.model.device)
        mse = self.loss(rendered_img.squeeze() * weighting_mask , self.ref_img * weighting_mask)
        # print(f"debugging mse scales: {mse}")
        dist = distance_loss(rendered_img.squeeze() * weighting_mask, self.ref_img * weighting_mask, gamma)
        ...
        position = th.zeros((1, 3), dtype=th.float32, device=self.model.device)  # (B, 3)
        ...
        pose_matrix = self.model.cTr_to_pose_matrix((self.buildcTr(self.cTr_train, self.cTr_nontrain)).unsqueeze(0)).squeeze()
        ...
        cylinder, angle = self.cylinder_loss(position, direction, pose_matrix, radius, self.fx, self.fy, self.px, self.py)
        ...
        if grid_search == False:
            ...
            return 10*ld1*mse + ld2*(1e-7)*dist + ld3*(1e-5)*app + 5*(1e-3)*pts + 10*(1e-3)*cylinder, angle
    ...
    def _overlay_masks(self, rendered_np, ref_np, ref_pts, proj_pts, detected_lines_params, projected_lines_params):
        """
        ...
        rendered_color = np.stack([0.5 * rendered_np, 0.8 * rendered_np, np.zeros_like(rendered_np)], axis=-1)
        ...
        if ref_pts != None: 
            for ref_pt in ref_pts:
                ...
                cv2.circle(overlay, (u_ref, v_ref), radius=5, color=(255, 0.6 * 255, 0.1 * 255), thickness=-1)  # Green
                
        u_ref, v_ref = int(center_ref_pt[0]), int(center_ref_pt[1])        
        cv2.circle(overlay, (u_ref, v_ref), radius=5, color=(255, 0.6 * 255, 0.1 * 255), thickness=-1)  # Green  BGR
        ...
        for proj_pt in proj_pts.squeeze():
            ...
            cv2.circle(overlay, (u_proj, v_proj), radius=5, color=(0.7, 0.8, 0), thickness=-1)  # Red
            
        u_ref, v_ref = int(center_proj_pt[0]), int(center_proj_pt[1])        
        cv2.circle(overlay, (u_ref, v_ref), radius=5, color=(0.7, 0.8, 0), thickness=-1)  # Green
        ...
        if detected_lines_params is not None:
            for line_params in detected_lines_params:
                ...
                cv2.line(overlay, (x1, y1), (x2, y2), (255, 0.6 * 255, 0.1 * 255), thickness=2)  # Blue
        ...
        if projected_lines_params is not None:
            for line_params in projected_lines_params:
                a, b = line_params
                (x1, y1), (x2, y2) = convert_line_params_to_endpoints(a, b, w, h)
                cv2.line(overlay, (x1, y1), (x2, y2), (0.7, 0.8, 0), thickness=2)  # green
