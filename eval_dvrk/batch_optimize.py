...
from cylinder_projection import projectCylinderTorch, transform_points, transform_points_b
...
def cylinder_loss_params(detected_lines, projected_lines):
    ...
    def line_difference(theta1, rho1, theta2, rho2):
        ...
        delta_theta = th.min(delta_theta, 2*math.pi - delta_theta)  # handle 2π periodicity
        delta_theta = th.min(delta_theta, math.pi - delta_theta)    # optional, if you want symmetrical ranges
    ...
    loss_1_0, theta_1_0 = line_difference(theta_det[:, 0], rho_det[:, 0],
                                          theta_proj[:, 0], rho_proj[:, 0])  # each => [B]
    loss_1_1, theta_1_1 = line_difference(theta_det[:, 1], rho_det[:, 1],
                                          theta_proj[:, 1], rho_proj[:, 1])  # each => [B]
    total_loss_1 = loss_1_0 + loss_1_1  # shape [B]

    # 5) Pairing 2: Det[0] ↔ Proj[1], Det[1] ↔ Proj[0]
    loss_2_0, theta_2_0 = line_difference(theta_det[:, 0], rho_det[:, 0],
                                          theta_proj[:, 1], rho_proj[:, 1])
    loss_2_1, theta_2_1 = line_difference(theta_det[:, 1], rho_det[:, 1],
                                          theta_proj[:, 0], rho_proj[:, 0])
    ...
    centerline_loss, _ = line_difference(theta_det_mean, rho_det_mean,
                                         theta_proj_mean, rho_proj_mean)  # shape [B]
...
def keypoint_loss_batch(keypoints_a, keypoints_b):
    ...
    dist_1 = th.norm(keypoints_a[:, 0] - keypoints_b[:, 0], dim=1) + \
             th.norm(keypoints_a[:, 1] - keypoints_b[:, 1], dim=1)

    # Permutation 2: A0->B1 and A1->B0
    dist_2 = th.norm(keypoints_a[:, 0] - keypoints_b[:, 1], dim=1) + \
             th.norm(keypoints_a[:, 1] - keypoints_b[:, 0], dim=1)
    ...
    centerline_loss = th.norm(th.mean(keypoints_a, dim=1) - th.mean(keypoints_b, dim=1), dim=1) # [B]
...
class BatchOptimize:
    ...
    def __init__(
        ...
        self.cTr_batch = th.nn.Parameter(cTr_batch.clone().detach()).to(self.device)  # shape (B,6)
    ...
    def readRefImage(self, ref_mask): 
        ...
        self.ref_mask_b = self.ref_mask.unsqueeze(0).expand(self.batch_size, self.ref_mask.shape[0], self.ref_mask.shape[1]) # [B, H, W]
    ...
    def batch_loss(
        ...
        weighting_mask = weighting_mask.unsqueeze(0).expand(B, *[-1 for _ in weighting_mask.shape])
        distance_map_ref = self.distance_map.unsqueeze(0).expand(B, *[-1 for _ in self.distance_map.shape]) # [B, H, W]
         
        # MSE
        mse_val = F.mse_loss(pred_mask_2d * weighting_mask, ref_mask_2d * weighting_mask, reduction= 'none') #[B, H, W]
        mse_val = mse_val.mean(dim=(1, 2)) # should be [B]
        # print(f"debugging mse scales: {mse_val}")
        # distance loss
        dist_val = distance_loss(pred_mask_2d*weighting_mask,  distance_map_ref*weighting_mask)  #[B]
        ...
        if ref_kps_2d is not None:
            ...
            [  0,         self.fy,  self.py],
            [  0,           0,           1       ]], device='cuda', dtype=self.joint_angles.dtype)
            ...
            p_local1 = th.tensor([0.0, 0.0004, 0.009]).to(self.joint_angles.dtype).to(self.model.device)
            p_local2 = th.tensor([0.0, -0.0004, 0.009]).to(self.joint_angles.dtype).to(self.model.device)
            p_img1 = get_img_coords_batch(p_local1, R_list[2], t_list[2], pose_matrix_b.squeeze().to(self.joint_angles.dtype), intr) 
            p_img2 = get_img_coords_batch(p_local2, R_list[3], t_list[3], pose_matrix_b.squeeze().to(self.joint_angles.dtype), intr)
            # They are both B, 2
            
            proj_pts = th.stack((p_img1, p_img2), dim=1) # [B, 2, 2]
         
            ref_kps_2d = ref_kps_2d.unsqueeze(0).expand(B,  ref_kps_2d.shape[0],  ref_kps_2d.shape[1]) #[B, 2, 2]
        ...
        cylinder_val = self.cylinder_loss_batch(
            position, direction, pose_matrix_b, radius, self.fx, self.fy, self.px, self.py
        )   # [B]
          
        # normalize
        ld1, ld2, ld3 = np.array([ld1, ld2, ld3])/np.linalg.norm(np.array([ld1, ld2, ld3]))
        
        item_loss = 10*ld1*mse_val + ld2*(1e-7)*dist_val + ld3*(1e-5)*app_val + 5*(1e-3)*pts_val + 10*(1e-3)*cylinder_val
        item_loss_mask = th.isnan(item_loss)
        if item_loss_mask.any():
            print(f"Found NaN in per_sample_loss for samples: {[mse_val], [dist_val], [app_val], [pts_val], [cylinder_val]}")
        
        return item_loss  # [B]
    
    # @th.compile
    def cylinder_loss_batch(self, position, direction, pose_matrix, radius, fx, fy, px, py):  
        ...
        cam_pts_3d_norm = th.nn.functional.normalize(cam_pts_3d_norm)   # NORMALIZE !!!!!!
      
        # print(f"checking shape of cylinder input: {cam_pts_3d_position.shape, cam_pts_3d_norm.shape}")  both [B,3]
        e_1, e_2 = projectCylinderTorch(cam_pts_3d_position, cam_pts_3d_norm, radius, fx, fy, px, py) # [B,2], [B,2]
        ...
        if self.det_line_params == None:
            ...
            # TODO visualize the detected lines in the first frame same as keypoint
            # line_image = cv2.cvtColor(ref_mask_np, cv2.COLOR_GRAY2BGR)
            
            # self.line_image = line_image   # just for visualize
            # self.longest_lines = longest_lines  # just for visualize
            ...
            # plt.imshow(ref_mask_np, cmap='gray')
            ...
            if longest_lines.shape[0] < 2:
                ...
                print("WARNING: Not enough lines found by Hough transform. Skipping cylinder loss.")
        ...
        B = self.batch_size
        detected_lines = detected_lines.unsqueeze(0).expand(B, detected_lines.shape[0], detected_lines.shape[1]) #[B, 2, 2]
...
# TODO finish the optimization pipeline
    ...
    def optimize_batch(
        ...
        for it in tqdm(range(iterations), desc= "optimizing batch samples...."):
            ...
            per_sample_loss = self.batch_loss(pred_masks_b, ref_masks_b, self.cTr_batch, self.ref_keypoints, ld1, ld2, ld3) #[B]
        ...
        if th.any(valid_mask):
            ...
            valid_indices = th.arange(len(final_losses), device= final_losses.device)[valid_mask]
            ...
            raise ValueError("All losses are NaN. Cannot determine the best initial cTr.")
