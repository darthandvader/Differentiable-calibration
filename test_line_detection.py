...
def detect_lines(mask, output = False):
    ...
    lines = cv2.HoughLinesP(edges, rho=0.5, theta = np.pi / 360, threshold=50, minLineLength=70, maxLineGap=10)
...
if __name__ == '__main__':
    ...
    lines = cv2.HoughLinesP(edges, rho=1, theta = np.pi / 180, threshold=60, minLineLength=50, maxLineGap=40)
    ...
    def lines_to_standard_form(lines):
        ...
        slopes = np.divide((y2 - y1), (x2 - x1), out=np.zeros_like(y1), where=~vertical_mask)
