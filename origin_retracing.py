...
from eval_dvrk.batch_optimize import BatchOptimize  # The class we just wrote
from eval_dvrk.optimize import Optimize            # Your single-sample class
import cv2
...
def parseArgs():
    ...
    parser.add_argument('--ref_img_file', type=str, default=os.path.join(data_dir, '00183.jpg')) # reference mask
    parser.add_argument('--joint_file', type=str, default=os.path.join(data_dir, 'joint_0183.npy')) # joint angles
    parser.add_argument('--jaw_file', type=str, default=os.path.join(data_dir, 'jaw_0183.npy')) # jaw angles 
    ...
    parser.add_argument('--batch_iters', type=int, default=100)   # Coarse steps per batch
    parser.add_argument('--final_iters', type=int, default=1000)  # Final single-sample refine
...
def main():
    ...
    for index in calibrate_indices:
        ...
        joint_angles = th.tensor(joint_angles_np, device=model.device, requires_grad=False, dtype=th.float32)
        ...
        ref_mask_np = (cv_img/255.).astype(np.float32)
        ...
        cTr_inits = []
        for i in range(N):
            camera_roll_local = th.empty(1).uniform_(0, 360)  # Random values in [0, 360]
            camera_roll = th.empty(1).uniform_(0, 360)  # Random values in [0, 360]
            azimuth = th.empty(1).uniform_(0, 360)      # Random values in [0, 360]
            elevation = th.empty(1).uniform_(90-60, 90-30)  # Random values in [90-25, 90+25]
            # elevation = 30
            distance = th.empty(1).uniform_(0.10, 0.17) 
            
            pose_matrix = model.from_lookat_to_pose_matrix(distance, elevation, camera_roll_local)
        ...
        ref_img = cv2.inRange(cv_img, np.ones(3) * 128, np.ones(3) * 255) / 255.
        ...
        if N <= bsz:
            ...
            batch_opt = BatchOptimize(
                ...
                fx=ctrnet_args.fx, fy=ctrnet_args.fy, px=ctrnet_args.px, py=ctrnet_args.py,
            ...
            print(f"[Single batch] best loss={best_loss_in_batch:.4f}, ctr={best_cTr_in_batch}")
        else:
            ...
            for start in range(0, N, bsz):
                ...
                final_batch_winners.append(best_cTr_in_batch)
                final_batch_winners_losses.append(best_loss_in_batch)
                print(f"[Batch range {start}-{end}] best in batch => loss={best_loss_in_batch:.4f} ctr={best_cTr_in_batch}")
        ...
        joint_angles = th.tensor(joint_angles_np, requires_grad=True, device= model.device, dtype= th.float32)
        ...
        single_opt.ref_keypoints = th.tensor(single_opt.ref_keypoints, device=single_opt.model.device, dtype=th.float32)
        single_opt.fx, single_opt.fy, single_opt.px, single_opt.py = ctrnet_args.fx, ctrnet_args.fy, ctrnet_args.px, ctrnet_args.py
        ...
        np.save(os.path.join(args.data_dir, f'optimized_ctr_{args.arm}.npy'), final_cTr_s.detach().cpu().numpy())
        np.save(os.path.join(args.data_dir, f'optimized_joint_angles_{args.arm}.npy'), joint_angles.detach().cpu().numpy())
